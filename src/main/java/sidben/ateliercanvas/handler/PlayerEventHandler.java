...
public class PlayerEventHandler
{
    ...
    {
        final boolean clientSide = event.target.worldObj.isRemote;
        ...
        if (!clientSide) {
            ...
            if (event.target instanceof EntityPainting) {
                ...
                if (handItem == null) {
                    ...
                    final EntityPainting currentPainting = (EntityPainting) event.target;
                    final boolean isSneaking = event.entityPlayer.isSneaking();
                    final World world = event.target.worldObj;


                    // Creates an array with all vanilla paintings
                    final EntityPainting.EnumArt[] artArray = EntityPainting.EnumArt.values();
                    ...
                    if (currentIndex > -1) {
                        ...
                        final EntityPainting newPainting = new EntityPainting(world, currentPainting.field_146063_b, currentPainting.field_146064_c, currentPainting.field_146062_d,
                                currentPainting.hangingDirection);
                         ...
                         */
                        for (@SuppressWarnings("unused") final EnumArt element : artArray) {
                            ...
                            if (newIndex >= artArray.length) {
                                newIndex = 0;
                            }
                            if (newIndex < 0) {
                                newIndex = artArray.length - 1;
                            }
